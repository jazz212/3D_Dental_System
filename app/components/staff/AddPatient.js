...
// patientId (uuid) is generated by the page on the server so step 2 can link
// to the patient. The record number shown to staff is assigned by the
// database when step 1 saves.
export default function AddPatient({ patientId }) {
  ...
  const handleSubmitIntake = async () => {
    ...
    setError(null);
    try {
      const patient = await createPatientWithHistory(patientId, profile, history);
      setRecordNumber(patient.patient_id);
      setStep(2);
    } catch (err) {
      if (err.code === "23505") {
        ...
        <div className="border w-full border-gray-300 overflow-hidden mt-2 rounded-lg">
          <div className="bg-blue-200 flex flex-wrap gap-2 items-center justify-between border border-gray-300 p-2 rounded-t-lg">
            ...
            <span className="bg-white border border-gray-300 rounded-lg px-2 py-1 text-sm text-gray-500">
              Rec: #{recordNumber || " assigned on save"}
            </span>
          </div>
